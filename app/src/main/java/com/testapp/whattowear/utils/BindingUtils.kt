...
fun getProperTextForEndTextView(view: TextView, value: Long) {
    view.text =
        if (value > 0) value.convertDateToReadableFormat() else view.resources.getString(R.string.btn_end_date_name)
}

@BindingAdapter("viewStartTripText")
fun getProperTextForStartTextView(view: TextView, value: Long) {
    view.text =
        if (value > 0) value.convertDateToReadableFormat() else view.resources.getString(R.string.btn_start_date_name)
}
